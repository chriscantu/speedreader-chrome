...
| **3 — `chrome.storage.session` does NOT survive restart** | **Manual** (real OS-level Chrome quit+relaunch) | **FAIL (as stated) + reproducer methodology flaw** | Chrome 138, startup = "Continue where you left off": a `session-sentinel` written pre-quit was **still present** after a full `Cmd+Q` + relaunch (sentinel `writtenAt` 452 s old → ruled out SW re-write; the SW survived the session *restore* and never re-ran its top-level write). The spec's binary claim "`session` does NOT survive restart" is therefore **false in a common default config**. Separately, the harness cannot measure the *cold-shutdown* case: `background.js` re-writes the sentinel on every SW wake (and opening SW DevTools wakes the SW), so a relaunched SW repopulates the value before it can be observed cleared. Check 3 needs the session write moved out of top-level load into an explicit trigger before a clean cold-boot verdict is possible. |

**Merge gate status: NOT cleared — check 3 FAILED as stated.** Checks 1 and 2 (automated, load-bearing) PASS. Check 3 falsifies the spec's binary "`session` is cleared on browser restart" premise: on Chrome 138 with restore-on-startup, `session` **survived**. Per §The Core Decision, this reopens Solution Design before any impl — the storage-backend rationale must be re-derived around the corrected fact that `session` survival is **config-dependent / unreliable** (survives session-restore; presumably cleared on a cold boot or "New Tab page" startup, but the current harness cannot prove the cold case). The spec stays `Status: Proposed`; **#232 does not merge and no IDB impl dispatches** until the rationale is revised and a fixed reproducer re-runs the cold-shutdown check.
