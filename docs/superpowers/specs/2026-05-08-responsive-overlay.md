...
- **Second popup-click while open.** Icon click acts as a unidirectional advance through the session cycle: `reading → paused → closed`. Resume from `paused` requires an explicit popup-button or overlay-control action — never an icon click — to avoid destructive surprises. The popup UI surfaces full pause/resume/close affordances when opened in a non-idle state per the hi-fi mock; the icon-click shortcut only fires the cycle's next-step action. Re-mount is impossible while the existing overlay is mounted; a fresh `start-read` only fires from `done` or no-Port states. See `docs/superpowers/specs/2026-05-08-messaging-contract.md` § Second popup-click while reading for the full state-transition spec.
